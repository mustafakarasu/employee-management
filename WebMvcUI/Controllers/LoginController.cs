...
namespace WebMvcUI.Controllers
{
    public class LoginController : Controller
    {
        ...
        public LoginController(IEntityRepository<User> userRepository,
                               IEntityRepository<Company> companyRepository,
                               IConfiguration configfuration)
        {
            _userRepository = userRepository;
            _companyRepository = companyRepository;
            _configuration = configfuration;
        }
        ...
        public async Task<IActionResult> Login(User user)
        {
            ...
            if (user.Email != null)
            {
                    ...
                    if (loginUser.HireDate.Date <= DateTime.Now.Date && terminationDate.Date > DateTime.Now.Date && PackageControl(loginUser.Id))
                    {
                        switch (loginUser.RoleId)
                        {
                            ...
                            case 2:
                                ...
                                {
                                    MailHelper.EndDateNoticeMail(loginUser, company, _configuration);
                            ...
                            case 3:
                                ...
                                {
                                    MailHelper.EndDateNoticeMail(_userRepository.Get(x => x.CompanyID == company.Id && x.RoleId == 2 && x.IsActive == true), company,_configuration);
                                    company.Informed = true;
                                    _companyRepository.Update(company);
                                }
                ...
                {
                    ViewBag.ErrorMessage = "Girdiğiniz email adresi veya şifre yanlıştır.";
                    return View();
                }
            }
            return View();
