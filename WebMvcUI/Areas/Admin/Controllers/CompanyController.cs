...
namespace WebMvcUI.Areas.Admin.Controllers
{
    ...
    public class CompanyController : Controller
    {
        ...
        public CompanyController(IEntityRepository<Company> companyRepository, IEntityRepository<Package> packageRepository, IEntityRepository<User> userRepository, IConfiguration configuration)
        {
        ...
        {
            if (_companyRepository.Get(x => x.Email == company.Email && x.IsActive == true) == null && _userRepository.Get(x => x.Email == company.User.Email && x.IsActive == true) == null && company.PackageStartingDate.Date >= DateTime.Now.Date && company.PackageStartingDate.Date <= DateTime.Now.AddMonths(3).Date)
            {
                ...
                MailHelper.SendMail(companyUser, company, _configuration);
                MailHelper.CompanyMail(company, companyUser, _configuration);
        ...
        {
            Company company = _companyRepository.Get(x => x.Id == updatedCompany.Id && x.IsActive == true);
